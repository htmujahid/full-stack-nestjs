...
import { CalendarEventController } from '../server/api/desk/calendar/calendar-event.controller';
import { CalendarEventService } from '../server/api/desk/calendar/calendar-event.service';
import { CalendarEvent } from '../server/api/desk/calendar/calendar-event.entity';
...
describe('Calendar events (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      imports: [TestModule, RouterModule.register([{ path: 'api/calendar-events', module: TestModule }])],
    }).compile();
